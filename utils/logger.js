...
async function logPagamento(client, tipo, user, detalhes = {}) {
    try {
        ...
        const defs = {
            ...
            entrega_manual: { title: "Entrega Manual Realizada", color: '#1ABC9C', emoji: "✍️" }, // <-- CORREÇÃO AQUI
    ...
    if (action === 'reenviar_dm') {
        try {
            ...
            if (!message || message === 'N/A') {
                return interaction.editReply({ content: '❌ Não há dados de entrega para reenviar a partir deste log.' });
            }

            const user = await interaction.client.users.fetch(userId);
            await user.send({ content: message });

            await interaction.editReply({ content: `✅ Mensagem de entrega reenviada com sucesso para ${user.tag}!` });
            
            await logAdmin(interaction.client, interaction.user, `Reenviou um produto manualmente para ${user.tag} a partir de um log de erro.`, 'Botão de Log');

        } catch (e) {
            console.error("Erro ao reenviar DM pelo log:", e);
            await interaction.editReply({ content: `❌ Falha ao reenviar DM. O usuário pode ter as DMs bloqueadas. Erro: ${e.message}` });
        }
    }
}
